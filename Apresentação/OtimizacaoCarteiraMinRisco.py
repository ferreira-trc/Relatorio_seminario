...
retorno_esperado_min = 0.002

# Restrição: 
modelo.add_constraint(modelo.sum(tickers[i] for i in tickers) == 1)
modelo.add_constraint(retorno_esperado >= retorno_esperado_min)


# Problema de otimização: Minimizar o risco do portfólio sujeito a um retorno esperado mínimo
modelo.minimize(risco_portfolio)
